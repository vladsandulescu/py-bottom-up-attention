        ...
        for probs, boxes, image_size in zip(probs_list, boxes_list, images.image_sizes):
            for nms_thresh in np.arange(0.3, 1.0, 0.1):
                instances, ids = fast_rcnn_inference_single_image(
                    ...
                    score_thresh=args.roi_score_thresh, nms_thresh=nms_thresh, topk_per_image=args.max_boxes
                )
...
if __name__ == "__main__":
    ...
    if args.output_type == 'tsv':
        output_path = os.path.join(args.output_path,
                                   ('d2_%d-%d_th%s/tsv' % (args.min_boxes,
                                                           args.max_boxes,
                                                           str(args.roi_score_thresh).replace('.', ''))))
